...
class DbFunction{
	...
	function login($loginid,$password){
      ...
      if(!ctype_alpha($loginid) || !ctype_alpha($password)){
        ...
        echo "<script>alert('loginId ou mot de passe erroné ')</script>";
	...
	if(false===$stmt){
		...
		if(!$rs)
		{
			echo "<script>alert('erreur Details')</script>";
			header('location:login.php');
		}
		else{
			
			header('location:ajout-cours.php');
		}
	}
	...
	function create_course($cshort,$cfull,$cdate){
			...
			echo "<script>alert('Selectionner abreviation')</script>";
			...
			echo "<script>alert('Selectionner section')</script>";
			...
			if(false===$stmt){
				...
				$stmt->execute();
				echo "<script>alert('AJOUTER AVEC SUCCEES')</script>";
					//header('location:login.php');
...
function register($cshort,$cfull,$fname,$mname,$lname,$gender,$gname,$ocp,$income,$category,$ph,
                  $nation,$mobno,$email,$country,$state,$city,$padd,$cadd,$session){
   ...
   $query = "INSERT INTO `registration` (`course`, `subject`, `fname`, `mname`, `lname`, `gender`, `gname`, `ocp`,
			...
			$stmt->bind_param('sssssssssssssssssssss',
					...
					$email,$country,$state,$city,$padd,$cadd,$session,$reg);
			$stmt->execute();
		   	echo "<script>alert('ENREGISTRER AVEC SUCCEES, VOTRE NUM D\'ENREGISTREMENT is $reg')</script>";
		 	//header('location:login.php');
...
function edit_course($cshort,$cfull,$udate,$id){
	...
	$stmt->execute();
    echo '<script>'; 
    echo 'alert("MODIFIER AVEC SUCCEES")'; 
    echo '</script>';
...
function edit_std($cshort,$cfull,$fname,$mname,$lname,$gender,$gname,$ocp,$income,$category,$ph,
                  $nation,$mobno,$email,$country,$state,$city,$padd,$cadd,$id){
  // echo $id;exit;
    $db = Database::getInstance();
	$mysqli = $db->getConnection();
	...
	$rc=$stmt->bind_param('sssssssssssssssssssi',$cshort,$cfull,$fname,$mname,$lname,$gender,$gname,$ocp,$income,$category,$ph,$nation,$mobno,$email,$country,$state,$city,$padd,$cadd,$id);
